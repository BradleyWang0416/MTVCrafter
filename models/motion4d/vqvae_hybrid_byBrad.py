...
class HYBRID_VQVAE(nn.Module):
    ...
    def forward(self, batch_dict, return_vq=False):
        ...
        if not self.vq.is_train:
            joint_feats, loss, indices = self.encdec_slice_frames(joint_feats, frame_batch_size=8, encdec=self.encoder, return_vq=return_vq, vision_feats=vision_feats)
        else:
            joint_feats, loss, perplexity = self.encdec_slice_frames(joint_feats, frame_batch_size=8, encdec=self.encoder, return_vq=return_vq, vision_feats=vision_feats)
        if return_vq:
            return joint_feats, loss
        joint_feats, _, _ = self.encdec_slice_frames(joint_feats, frame_batch_size=2, encdec=self.decoder, return_vq=return_vq)
    ...
    def encdec_slice_frames(self, joint_feats, frame_batch_size, encdec, return_vq, vision_feats=None):
        ...
        if encdec == self.encoder and self.vq is not None and not self.vq.is_train:
            joint_output, loss, indices = self.vq(torch.cat(joint_output, dim=2), return_vq=return_vq)
            return joint_output, loss, indices
        elif encdec == self.encoder and self.vq is not None and self.vq.is_train:
            joint_output, loss, preplexity = self.vq(torch.cat(joint_output, dim=2))
            return joint_output, loss, preplexity
        else:
            return torch.cat(joint_output, dim=2), None, None
